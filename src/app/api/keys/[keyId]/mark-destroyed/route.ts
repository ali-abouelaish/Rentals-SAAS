...
export async function POST(
  ...
  try {
    ...
    if (closeErr) {
      console.error("[api/keys/mark-destroyed] close", closeErr);
      return NextResponse.json({ error: "Failed to mark key destroyed" }, { status: 500 });
    }
    ...
    if (statusErr) {
      console.error("[api/keys/mark-destroyed] status update", statusErr);
      return NextResponse.json({ error: "Failed to mark key destroyed" }, { status: 500 });
    }

    return NextResponse.json({ ok: true, status: "destroyed" });
  } catch (err) {
    if (err instanceof AssertionError) {
      return NextResponse.json({ error: err.message }, { status: err.status });
    }
    console.error("[api/keys/mark-destroyed]", err);
    return NextResponse.json({ error: "Failed to mark key destroyed" }, { status: 500 });
  }
}
