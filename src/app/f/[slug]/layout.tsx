...
export default async function FormPublicLayout({ children, params }: LayoutProps) {
  const supabase = createSupabaseAdminClient();
  const { data } = await supabase
    .from("forms")
    .select("name, tenant:tenants(name, branding:tenant_branding_settings(brand_name))")
    .eq("public_slug", params.slug)
    .eq("is_active", true)
    .single();

  const tenant = data?.tenant as unknown as TenantRow;
  const brandName = tenant?.branding?.brand_name ?? tenant?.name ?? "Agency";
  ...
  return (
    ...
    >
      {/* atmosphere — layered radial washes */}
      <div
        aria-hidden
