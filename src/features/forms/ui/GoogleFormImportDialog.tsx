...
import { importGoogleForm } from "../actions/form-import";
...
interface GoogleFormImportDialogProps {
  formId: string;
  open: boolean;
...
export function GoogleFormImportDialog({
  formId,
  open,
  onOpenChange,
  onImported,
}: GoogleFormImportDialogProps) {
  ...
  const handleParse = () => {
    ...
    startParsing(async () => {
      try {
        const questions = await importGoogleForm(rawText);
  ...
  const handleImport = () => {
    startImporting(async () => {
      try {
        await bulkCreateFormQuestions(formId, parsed);
        toast.success(`${parsed.length} question${parsed.length === 1 ? "" : "s"} imported`);
        onImported(parsed as unknown as FormQuestion[]);
        handleClose();
      } catch (err) {
        ...
        {step === "paste" ? (
          <div className="space-y-4 mt-1">
            <p className="text-sm text-foreground-secondary">
              Paste the text of your Google Form questions below. The AI will detect each question,
