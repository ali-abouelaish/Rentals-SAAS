  ...
  const onSubmit = (values: FormQuestionValues) => {
    startTransition(async () => {
      try {
        ...
        onAdded(question as unknown as FormQuestion);
      } catch {
    ...
    >
      ...
      >
        {isInfo ? (
          <textarea
            ...
            placeholder="e.g. We will contact you within one working day to discuss next steps."
            autoFocus
          />
        ) : (
          <input
      ...
      {!isInfo && (
        <FormField label="Required">
          <select
            {...register("is_required", { setValueAs: (v) => v === "true" })}
            className={selectCls}
          >
          ...
          <>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {question.is_required && (
                <span className="text-[10px] font-medium text-red-600 bg-red-50 rounded px-1.5 py-0.5">
                  Required
                </span>
              )}
...
export function QuestionEditor({ formId, questions, onQuestionsChange }: QuestionEditorProps) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    reorderFormQuestions(reordered.map((q) => ({ id: q.id, sort_order: q.sort_order }))).catch(
      () => {
